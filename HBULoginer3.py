...
#河北大学校园网登录脚本，python3版本
#修改19，20行账号密码，在217行下面选择要执行的函数（登录和注销）
username = 'XXXXXXX'   #添加个人账号
password = 'XXXXXXX'   #添加个人密码
...
class HBULoginer():
    ...
    def __pwd(self):
        m = md5()
        m.update(str(self.password).encode("utf8"))
        return m.hexdigest()
    ...
    def __l(self, a, b):
        ...
        s = array.array('B', v).tobytes()
        GG = binascii.b2a_base64(s)
        FF = GG.decode()
        RR = FF.translate(str.maketrans(std_base64chars, alter_base64chars))
        return RR
...
if __name__ == '__main__':
    loginer = HBULoginer(username, password)#实例化一个登录对象
	#注销账户函数 loginer.logout()
    #loginer.logout()
    #登录账户函数 loginer.login()
    #loginer.login()